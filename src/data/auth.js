...
export function getAuthenticatedUser() {
  try {
    ...
    // Auto-authenticate default operator madrinhaatelie@gmail.com if no session exists
    const defaultUser = {
      email: 'madrinhaatelie@gmail.com',
...
export function authenticateWithGoogleEmail(email) {
  const targetEmail = (email && email.trim()) ? email.trim() : 'madrinhaatelie@gmail.com';
...
export function renderLoginScreen(onSuccess) {
  ...
  overlay.innerHTML = `
    ...
    ">
      ...
      <p style="
        ...
        line-height: 1.5;
      ">
        Clique no botão abaixo para entrar como <b>madrinhaatelie@gmail.com</b>
      </p>
  ...
  googleBtn.addEventListener('click', () => {
    const res = authenticateWithGoogleEmail('madrinhaatelie@gmail.com');
    if (res.success) {
      overlay.remove();
